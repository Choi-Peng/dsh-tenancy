...
3. **事件流隔离** — 经 `dsh-api-gateway` 的 `/api/remote.mux` 多路复用流做开闸判定与下行逐帧过滤(`$events` 广播/waterfall ask 帧、`workspace/follow` 视图帧、`session/control` 增量),未授权会话**零帧**泄漏
