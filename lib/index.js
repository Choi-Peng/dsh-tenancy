...
export function apply(ctx, config) {
  ...
  const memberRoot = (() => {
    ...
    if (!raw) return null;
    return expandHomeDir(raw, homedir()); // 已 realpath 归一(根自身是符号链接时围栏不失效)
  })();

  // 尽力确保围栏根存在;失败不阻塞启动(后续请求按目录缺失自然报错)。
  // 建好后再 realpath 一次:配置指向尚不存在的目录时,首轮 expandHomeDir 只能
  // 退回词法路径,不补这一步就会留下未归一的根,留下符号链接逃逸空间。
  if (memberRoot) {
    ...
    try {
      ...
      if (!isAdminOf(principal) && envelope) {
        ...
        if (verdict.deny) {
          ...
          return deny(res, 403, `tenancy: path outside workspace root (${memberRoot})`);
        }
        if (verdict.changed) {
