...
// ask 类帧),记入索引;/api/respond 影子路由据此做归属校验。
  ...
  async function gatedHandler(method, req, res) {
    try {
      ...
      if ((method === 'session.create' || method === 'session.fork')
        ...
        if (newId) {
          ...
          pendingSessions.set(newId, {
            ...
            title: typeof payload.title === 'string' ? payload.title : (parentRec?.title ?? ''),
            updatedBy: principal.user,
            updatedAt: Date.now()
          });
      ...
      // payload.title 是用户设定的原始标题;response.value.title 是规范化后的标题。
      if (method === 'session.rename' && upstream.ok && body?.result?.ok !== false && sid) {
        const renamedTitle = body?.result?.value?.title ?? payload?.title ?? '';
      ...
      // payload.title 是新标题;response.value.workspace.title 是规范化后的标题。
      if (method === 'workspace.rename' && upstream.ok && body?.result?.ok !== false) {
        const wsId = payload?.workspaceId;
        const renamedTitle = body?.result?.value?.workspace?.title ?? payload?.title ?? '';
