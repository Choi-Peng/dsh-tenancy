...
import { confineToRoot, expandHomeDir, isTrustedApiRequest, recordReadable, recordWritable, safeDecode, safeSegment } from './util.js';
import { workspaceCreatable, workspaceSharedUsers, workspaceVisible } from './util.js';
...
export function apply(ctx, config) {
   ...
   */
  async function createPersonalWorkspace(username) {
  ...
  async function confineMemberPayload(method, payload, principal) {
    ...
    switch (method) {
      ...
      case 'host.createDirectory': {
        ...
        const personalRoot = await personalRootOf(principal.user);
        if (!personalRoot) return { deny: true, reason: 'path-outside-root' };
        const baseDir = typeof payload.path === 'string' && payload.path.trim() !== '' ? payload.path : personalRoot;
        const target = confineToRoot(personalRoot, pathResolve(baseDir, seg));
      ...
      case 'host.listDirectory': {
        ...
        const confined = requested ? confineToRoot(personalRoot, requested) : null;
        if (!confined || confined !== requested) {
          payload.path = confined ?? personalRoot;
          return { deny: false, changed: true };
        }
        return { deny: false };
      }
  ...
  async function gatedHandler(method, req, res) {
    try {
      ...
      if (method === 'host.listDirectory' && !isAdminOf(principal)
        ...
        const personalRoot = await personalRootOf(principal.user);
        body.result.value.home = personalRoot ?? memberRoot;
        return sendJson(res, upstream.status, jsonBuf(body));
      }
