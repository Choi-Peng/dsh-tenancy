...
export function apply(ctx, config) {
  ...
  // 放行的 waterfall ask 帧按 eventId → sessionId 记入索引,/api/$events/result
  // 影子路由据此做归属校验;该索引只由"确实下发给某主体的帧"填充,成员浏览器里
  // 出现不了的 eventId 自然不会入索引(纵深防御)。
  // ⚠ 必须对**所有**主体记录(含 admin):admin 在 filterEvent 里整帧直通,
  //   若把记录写在 admin 旁路之后,admin 自己提问/授权时会因 eventId 未入索引
  //   被自己这道门 403(rpcid-unknown-or-expired),浏览器侧表现为应答 RPC 失败
  //   → $events 流被 answer() 失败连带中止 → [connection] connection lost retry,
  //   而提问依旧挂起。
  ...
  const filterEvent = (principal, endpoint, value) => {
    ...
    // ask 帧的 eventId → 会话归属索引必须在 admin 旁路**之前**记录:admin 整帧直通,
    // 漏记会让 admin 自己应答提问/授权时被 /api/$events/result 门 403(见 noteRpc 注释)。
    if (value.type === 'waterfall') {
      const askSid = value.agentId != null && value.agentId !== '' ? String(value.agentId) : null;
      if (askSid && canSeeUser(principal, askSid) && typeof value.eventId === 'string') {
        noteRpc(value.eventId, askSid);
      }
    }
