...
/**
 * ACL 记录的可读判定(纯谓词;admin 短路在调用方)。记录来源既可以是
 * 已落盘的 acl.json,也可以是 P7 的内存 pending 表——两者同形,判定同构。
 */
export function recordReadable(rec, user) {
  if (!rec) return false;
  return rec.owner === user || rec.mode === 'team-read' || rec.mode === 'team-rw'
    || (rec.readers?.includes(user) ?? false);
}

/** ACL 记录的可写判定(纯谓词;admin 短路在调用方)。 */
export function recordWritable(rec, user) {
  if (!rec) return false;
  return rec.owner === user || rec.mode === 'team-rw'
    || (rec.writers?.includes(user) ?? false);
}

//#region 工作区共享(P9:工作区把会话围栏从 owner-only 扩展到 sharedUsers)

/** 读工作区记录的共享用户表(缺省 [];兼容旧记录无该字段)。 */
export function workspaceSharedUsers(wrec) {
  return Array.isArray(wrec?.sharedUsers) ? wrec.sharedUsers : [];
}

/**
 * 工作区对某用户是否可「新增会话」:owner 或共享用户。这是 session.create 的
 * workspaceId 门——共享用户可在共享工作区新建会话;仅被共享了会话但工作区未
 * 共享时(见 requirement 5),用户不是 owner/共享用户,故这里返回 false,
 * 从而无法在侧栏对未共享工作区新增会话。
 */
export function workspaceCreatable(wrec, user) {
  if (!wrec) return false;
  return wrec.owner === user || workspaceSharedUsers(wrec).includes(user);
}

/**
 * 工作区是否对用户可见(侧栏/列表):owner、共享用户,或工作区内存在用户
 * 可读的会话(仅共享会话但未共享工作区也要让用户看到该工作区名与该会话)。
 * 管理员短路在调用方。
 */
export function workspaceVisible(wrec, canReadAnySession, user) {
  if (!wrec) return false;
  if (wrec.owner === user) return true;
  if (workspaceSharedUsers(wrec).includes(user)) return true;
  return Boolean(canReadAnySession);
}

/**
 * P13:目标路径是否落在某已登记工作区内;返回覆盖该路径的**最深**工作区记录
 * (嵌套工作区取最内层),未覆盖返回 null。用于共享工作区的文件访问放行——
 * 浏览/建目录请求先查这条,命中可新建会话的工作区即放开到该工作区根。
 */
export function workspaceCoveringRecord(workspaces, path) {
  if (!workspaces || typeof path !== 'string' || path === '') return null;
  let best = null;
  for (const rec of Object.values(workspaces)) {
    const p = rec?.path;
    if (typeof p !== 'string' || p === '') continue;
    if (path === p || path.startsWith(p.endsWith('/') ? p : `${p}/`)) {
      if (!best || p.length > (best.path ?? '').length) best = rec;
    }
  }
  return best;
}
...
/**
 * P13:共享工作区中新建会话的**默认可读者** = 工作区 owner + sharedUsers - 创建者。
 * 工作区尚未共享(sharedUsers 为空)时返回 null → 维持默认私有。
 * 「创建时刻捕获」语义由调用方保证:创建会话时读取当时的 sharedUsers 落进会话
 * ACL——因此共享**前**已建的会话保持私有,共享**后**新建的默认对参与者可读,
 * owner 之后可随时经会话共享对话框改回私有。只给读不给写:写仍需 owner 授权。
 */
export function workspaceDefaultSessionReaders(wrec, creator) {
  if (!wrec) return null;
  const shared = workspaceSharedUsers(wrec);
  if (shared.length === 0) return null;
  const set = new Set(shared);
  if (wrec.owner && wrec.owner !== creator) set.add(wrec.owner);
  set.delete(creator);
