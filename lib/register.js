...
//   · 用户库写入用 O_APPEND 单次 write(保 inode,Authelia 文件监听不失效);
...
export function hashArgon2(autheliaBin, password) {
  return new Promise((resolvePromise, rejectPromise) => {
    execFile('/bin/sh', ['-c', 'exec "$AUTHELIA_BIN" crypto hash generate argon2 --password "$REG_PW"'], {
      ...
      const m = /\$argon2(?:id)?\$[A-Za-z0-9$+/=.]+/.exec(String(stdout));
...
(function () {
  ...
  var T = {
    ...
    success: ['注册成功!点击右上角登录。', 'Success! Continue to sign in.']
  };
  if (!zh) {
  ...
  document.getElementById('reg').addEventListener('submit', function (ev) {
    ...
    fetch('/register/api', {
      ...
      if (d && d.ok) {
        ...
        a.href = '/auth/?rd=%2F'; a.textContent = zh ? '去登录' : 'Continue';
...
export function createRegisterRoutes(deps) {
  // deps: { config, store, audit, logger }
