...
// P12:注册成功后的副作用链先开通同名系统用户(deps.provisionSystemUser,nologin
//   不可登录、文件权限仅限个人工作区),再建个人工作区(deps.createPersonalWorkspace
 ...
 * P9:注册成功后调用 deps.createPersonalWorkspace?(username) 为其在 `~/dsh/` 下创建
 * 以用户名为名的工作区(尽力而为,失败仅记日志并返回 ok——注册主体已完成)。
 * P12:创建工作区前先调用 deps.provisionSystemUser?(username) 开通同名系统用户
 * (nologin、无密码,文件权限仅限个人工作区;同样尽力而为,失败不翻转注册结果),
 * 其返回的 uid/gid 交给 createPersonalWorkspace 做目录归属。
 */
export async function registerUser(deps, input) {
  ...
  if (outcome.ok) {
    ...
    // P12:先开通同名系统用户(nologin、无密码,文件权限仅限个人工作区)。
    // 尽力而为:失败不翻转注册结果,失败详情由 provisionSystemUser 内部审计/告警。
    let sysAccount = null;
    try {
      if (typeof deps.provisionSystemUser === 'function') {
        sysAccount = await deps.provisionSystemUser(username);
      }
    } catch (e) {
      deps.logger?.error?.(`tenancy: 注册后为 ${username} 开通系统用户失败: ${e?.message ?? e}`);
    }
    // P9:注册成功后为其建个人工作区(~/dsh/<username>, owner=用户;P12:归属系统用户)。
    // 尽力而为:失败不翻转注册结果,仅告警;用户首次登录时仍可见(目录已建,可自行在用户中心补建)。
    try {
      if (typeof deps.createPersonalWorkspace === 'function') {
        const created = await deps.createPersonalWorkspace(username, sysAccount);
        if (created) audit.record({ actor: username, action: 'workspace.create', target: 'workspace', workspaceId: created.workspaceId, reason: 'register-auto', via: actorHint });
        else deps.logger?.warn?.(`tenancy: 已注册 ${username} 但未创建个人工作区`);
      }
    } catch (e) {
      deps.logger?.error?.(`tenancy: 注册后为 ${username} 建个人工作区失败: ${e?.message ?? e}`);
    }
...
export function createRegisterRoutes(deps) {
  // deps: { config, store, audit, logger, isTrusted, createPersonalWorkspace? }
      ...
      if (req.method === 'POST' && pathname === '/api') {
        ...
        const result = payload
          ? await registerUser({
            ...
            actorHint: `ip:${ip}`,
            logger: deps.logger,
            // P12:必须透传,否则注册链路会静默跳过系统用户开通(自测 ③ 直调
            // registerUser 时依赖注入齐全,暴露不了这个漏接)
            provisionSystemUser: deps.provisionSystemUser,
            createPersonalWorkspace: deps.createPersonalWorkspace
          }, payload)
