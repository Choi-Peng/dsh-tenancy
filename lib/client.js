...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var en = {
      ...
      inviteUsedBy: 'by',
      ucOpen: 'User center',
      ucFloatLabel: 'Floating entry',
      ucFloatHint: 'Off: the user-center entry stays fixed at the top center of the page. On: drag it anywhere; the position is remembered in this browser.',
      ...
      wsSessions: 'sessions',
      wsCreated: 'created',
      wsShared: 'Shared with',
      wsShareLabel: 'Share',
      wsShareTitle: 'Workspace sharing',
      wsSharedUsersLabel: 'Shared users (comma-separated usernames)',
      wsSave: 'Save',
      wsSaved: 'Saved.',
      wsShareLoadFailed: 'Failed to load workspace sharing.',
      wsNotOwner: 'Only the workspace owner can manage sharing.',
      wsShareSaveFailed: 'Failed to save workspace sharing.',
      wsYou: 'you',
      ucSessions: 'Sessions',
      ...
      restartTitle: 'Restart dsh-web (via pm2)',
      restartConfirm: 'Restart dsh-web? All online sessions will be interrupted.',
      restartOk: 'Restarting…',
      restartDone: 'Restart triggered',
    ...
    var zh = {
      ...
      ucOpen: '用户中心',
      ucFloatLabel: '浮动入口',
      ucFloatHint: '关闭:入口固定在页面顶部居中;开启:可拖动到任意位置,位置在本浏览器内记住。',
      ucIdentity: '当前身份',
      ucWorkspaces: '工作区管理',
      ...
      wsShareLabel: '共享',
      wsShareTitle: '工作区共享',
      wsSharedUsersLabel: '共享用户(逗号分隔用户名)',
      wsSave: '保存',
      wsSaved: '已保存。',
      wsShareLoadFailed: '工作区共享信息加载失败。',
      wsNotOwner: '仅工作区所有者可管理共享。',
      wsShareSaveFailed: '工作区共享保存失败。',
      wsYou: '我',
      ucSessions: '会话管理',
      ucPassword: '修改密码',
      pwCurrent: '当前密码',
      ...
      ucLogout: '登出',
      restartLabel: '重启服务',
      restartTitle: '重启 dsh-web(通过 pm2)',
      restartConfirm: '确定重启 dsh-web?所有在线用户会话将中断。',
      restartOk: '重启中…',
      restartDone: '已触发重启',
      restartFailed: '重启失败',
    };
    ...
    /** ── 工作区共享对话框(owner/admin 管理 sharedUsers)────────────────── */
    function WorkspaceShareDialog(props) {
      var workspaceId = props.workspaceId; var t = props.t; var me = props.me; var onClose = props.onClose; var onSaved = props.onSaved;
      var state = react.useState({ loading: true, failed: false, sharedUsers: [] });
      var info = state[0]; var setInfo = state[1];
      ...
      react.useEffect(function () {
        var alive = true;
        api('GET', '/tenancy/workspaces/' + encodeURIComponent(workspaceId) + '/share').then(function (res) {
          if (!alive) return;
          setInfo({ loading: false, failed: !res.ok, sharedUsers: res.ok && Array.isArray(res.data.sharedUsers) ? res.data.sharedUsers : [] });
        }).catch(function () { if (alive) setInfo({ loading: false, failed: true, sharedUsers: [] }); });
        return function () { alive = false; };
      }, [workspaceId]);

      function onSave() {
        var users = parseList(info.sharedUsers.join(', '));
        setBusy(true); setMsg('');
        api('POST', '/tenancy/workspaces/' + encodeURIComponent(workspaceId) + '/share', { sharedUsers: users }).then(function (res) {
          setBusy(false);
          if (res.ok) { setInfo({ loading: false, failed: false, sharedUsers: res.data.sharedUsers || [] }); setMsg(t('wsSaved')); if (onSaved) onSaved(); }
          else { setMsg(res.data && res.data.error ? String(res.data.error) : t('wsShareSaveFailed')); }
      ...
      var body;
      if (info.loading) body = react.createElement('p', { style: hintStyle }, '…');
      else if (info.failed) body = react.createElement('p', { style: hintStyle }, t('wsShareLoadFailed'));
      else body = react.createElement('div', { style: { display: 'flex', flexDirection: 'column' } },
        react.createElement('div', { style: fieldStyle },
          react.createElement('label', { style: labelStyle, htmlFor: 'tenancy-ws-shared' }, t('wsSharedUsersLabel')),
          react.createElement('input', {
            id: 'tenancy-ws-shared', style: controlStyle, value: info.sharedUsers.join(', '),
            onChange: function (e) { setInfo(Object.assign({}, info, { sharedUsers: parseList(e.target.value) })); }
          })
        ),
        ...
        msg ? react.createElement('p', { style: hintStyle }, msg) : null
      );
      ...
      return react.createElement('div', {
        ...
        react.createElement('div', {
          role: 'dialog', 'aria-label': t('wsShareTitle'),
          style: { background: 'var(--dsw-alias-bg-layer-2, #fff)', color: 'var(--dsw-alias-label-primary)', borderRadius: '12px', minWidth: '340px', maxWidth: '460px', maxHeight: '80vh', overflow: 'auto', padding: '18px', boxShadow: '0 8px 32px rgba(0,0,0,0.25)' }
        },
          react.createElement('div', { style: { display: 'flex', alignItems: 'center', marginBottom: '6px' } },
            react.createElement('span', { style: { flex: 1, fontSize: '15px', fontWeight: '600' } }, t('wsShareTitle')),
            react.createElement('button', {
              type: 'button', 'aria-label': t('closeLabel'), onClick: onClose,
              style: { background: 'transparent', border: '0', cursor: 'pointer', color: 'var(--dsw-alias-label-secondary)', fontSize: '16px', lineHeight: 1 }
            }, '\u00d7')
          ),
          react.createElement('code', { style: { display: 'block', fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '8px' } }, workspaceId),
          body
        )
      );
    }
    ...
    function WorkspaceList(props) {
      ...
      var info = state[0]; var setInfo = state[1];
      var meState = react.useState(null); var me = meState[0]; var setMe = meState[1];
      var shareForState = react.useState(null); var shareFor = shareForState[0]; var setShareFor = shareForState[1];
      ...
      var load = react.useCallback(function () {
        ...
        return Promise.all([wsPromise, metaPromise]).then(function (results) {
          var res = results[0];
          var meta = results[1] || {};
          ...
          setInfo({
            ...
            rows: rows.map(function (rec) {
              ...
              return {
                ...
                sharedUsers: Array.isArray(rec.sharedUsers) ? rec.sharedUsers : [],
                title: title,
                count: m ? m.count : 0
              };
            })
          });
        }).catch(function () { setInfo({ loading: false, failed: true, rows: [] }); });
      }, []);
      ...
      var shareBtnStyle = { flex: 'none', background: 'transparent', color: 'var(--dsw-alias-label-secondary)', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '6px', padding: '1px 8px', fontSize: '11px', cursor: 'pointer' };
      ...
      if (info.loading) body = react.createElement('p', { style: { margin: 0, fontSize: '12px', color: 'var(--dsw-alias-label-tertiary)' } }, t('ucLoading'));
        ...
        var canManage = Boolean(me && (me.admin || rec.owner === me.user));
        return react.createElement('div', { key: rec.workspaceId, style: rowStyle },
          ...
          react.createElement('span', { style: chipStyle }, rec.owner === me.user ? t('wsYou') : rec.owner),
          rec.sharedUsers.length > 0 ? react.createElement('span', { style: chipStyle, title: t('wsShared') + ': ' + rec.sharedUsers.join(', ') }, '↗ ' + rec.sharedUsers.length) : null,
          ...
          canManage ? react.createElement('button', {
            type: 'button', style: shareBtnStyle,
            onClick: function () { setShareFor(rec.workspaceId); }
          }, t('wsShareLabel')) : null
        );
      }));
      ...
      return react.createElement('div', null,
        ...
        body,
        shareFor ? react.createElement(WorkspaceShareDialog, {
          workspaceId: shareFor, t: t, me: me, onClose: function () { setShareFor(null); }, onSaved: refresh
        }) : null
      );
    }
    ...
    /** ── 顶部居中常驻用户中心入口(fixed 定位,挂载于 root 级常驻槽位)──────
     *  默认「浮动入口」关闭:入口固定在 body 顶部居中、不可拖动;
     *  在 设置 → 插件 → 多租户 打开后,才启用拖拽与坐标持久化。 */
    /** ── 浮动开关:模块级共享状态(设置卡片写入,入口圆片订阅)─────────────── */
    var UC_FLOAT_KEY = 'tenancy.uc.float';
    var ucFloatEnabled = loadUcFloat();
    var ucFloatListeners = [];

    function loadUcFloat() {
      try { return localStorage.getItem(UC_FLOAT_KEY) === '1'; } catch (e) { return false; }
    }

    /** 写入开关并广播;默认关闭(固定顶部居中),打开后可自由拖动。 */
    function setUcFloat(on) {
      ucFloatEnabled = Boolean(on);
      try {
        if (ucFloatEnabled) localStorage.setItem(UC_FLOAT_KEY, '1');
        else localStorage.removeItem(UC_FLOAT_KEY);
      } catch (e) { /* 存储不可用不致命 */ }
      for (var i = 0; i < ucFloatListeners.length; i += 1) {
        try { ucFloatListeners[i](ucFloatEnabled); } catch (e) { /* 单个订阅者失败不影响其它 */ }
      }
    }

    /** 订阅浮动开关:返回 [enabled, setEnabled],设置卡片与入口圆片共享同一状态。 */
    function useUcFloat() {
      var state = react.useState(ucFloatEnabled);
      var enabled = state[0]; var setEnabled = state[1];
      react.useEffect(function () {
        function onChange(v) { setEnabled(v); }
        ucFloatListeners.push(onChange);
        setEnabled(ucFloatEnabled); // 挂载时对齐模块状态(设置改动可能早于本组件挂载)
        return function () {
          var i = ucFloatListeners.indexOf(onChange);
          if (i >= 0) ucFloatListeners.splice(i, 1);
        };
      }, []);
      return [enabled, setUcFloat];
    }

    /** ── 拖拽位置持久化:pos 统一存入口圆片左上角坐标,面板由它推导 ─────── */
    var UC_PILL_PANEL_GAP = 42; // 面板默认 top:54px 与圆片 top:12px 的间距
    // 面板高度估值(样式为 maxHeight:calc(100vh - 70px));仅用于纵向越界夹取,
    // 避免把 window.innerHeight 当元素高度传入时 maxY 恒为 0(面板被钉在顶部)。
    var UC_PANEL_EST_H = 520;
    ...
    /** 未拖拽过时的兜底坐标(顶部居中);正常渲染走 CSS 居中,不依赖此估算。 */
    function defaultUcPos() {
      return { x: Math.max(8, Math.round((window.innerWidth - 160) / 2)), y: 12 };
    }

    /** 由圆片坐标推导面板左上角(圆片正下方,间距 UC_PILL_PANEL_GAP)。 */
    function panelPosFromPill(pos) {
      return clampPos({ x: pos.x, y: pos.y + UC_PILL_PANEL_GAP }, 380, UC_PANEL_EST_H);
    }
    ...
    function UserCenter(props) {
      ...
      // 浮动开关(默认关闭):关闭时入口固定顶部居中且不可拖动;pos 仅在浮动态生效。
      var floatState = useUcFloat();
      var floating = floatState[0];
      // 面板拖拽位置;null = 未拖过,用顶部居中锚点定位;dragging 期间禁止面板滚动溢出
      ...
      function startPanelDrag(e) {
        ...
        // 保证关闭面板后入口圆片落回面板最后一次位置上方。未拖拽过时面板由
        // CSS 居中锚定,没有对应坐标,直接量取真实矩形起手,避免首帧跳动。
        var liveRect = e.currentTarget && e.currentTarget.parentNode && e.currentTarget.parentNode.getBoundingClientRect
          ? e.currentTarget.parentNode.getBoundingClientRect() : null;
        var base = pos ? panelPosFromPill(pos)
          : liveRect ? { x: liveRect.left, y: liveRect.top }
            : { x: defaultUcPos().x, y: 12 + UC_PILL_PANEL_GAP };
        dragRef.current = {
        ...
        function onMove(ev) {
          ...
          d.moved = true;
          var next = clampPos({ x: d.baseX + dx, y: d.baseY + dy }, 380, UC_PANEL_EST_H);
      ...
      // 仅浮动态挂载本处理器;固定态走 onClick 开合,不会误改坐标。
      ...
      // 入口定位:默认固定 body 顶部居中(left:50% + translateX(-50%));
      // 浮动开启且拖过之后才切到持久化坐标(像素 left/top,不再带 transform)。
      var pillStyle = {
        position: 'fixed', top: '12px', zIndex: 9500,
      ...
      var pillPos = floating && pos ? clampPos(pos, 160, 34) : null;
      if (pillPos) { pillStyle.left = pillPos.x + 'px'; pillStyle.top = pillPos.y + 'px'; }
      else { pillStyle.left = '50%'; pillStyle.transform = 'translateX(-50%)'; }
      var avatarStyle = {
      ...
      return react.createElement('div', { 'data-uc': 'true' },
        react.createElement('div', {
          ...
          style: pillStyle,
          // 固定态:单击开合;浮动态:拖拽处理器内部区分点击与拖动。
          onMouseDown: floating ? startPillDrag : undefined,
          onClick: floating ? undefined : function () { setOpen(function (v) { return !v; }); },
        ...
        open ? react.createElement('div', {
          ...
          react.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', cursor: floating ? 'move' : 'default', userSelect: 'none' }, onMouseDown: floating ? startPanelDrag : undefined },
    ...
    function TenancyCard(props) {
      ...
      var open = openState[0]; var setOpen = openState[1];
      // 浮动入口开关:默认关闭 → 用户中心入口固定在页面顶部居中;开启后可自由拖动。
      var floatState = useUcFloat();
      var floatOn = floatState[0]; var setFloatOn = floatState[1];
      ...
      // 浮动入口开关:自绘 switch(宿主 primitives 未注入本插件,保持卡片原生风格)。
      var floatRow = react.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '12px', borderTop: '1px solid var(--dsw-alias-border-l2)', marginTop: '4px', padding: '10px 0' } },
        react.createElement('span', { style: { display: 'flex', flexDirection: 'column', gap: '2px', flex: 1, minWidth: 0 } },
          react.createElement('span', { style: { fontSize: '13px', color: 'var(--dsw-alias-label-primary)' } }, t('ucFloatLabel')),
          react.createElement('span', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-tertiary)', lineHeight: '1.5' } }, t('ucFloatHint'))
        ),
        react.createElement('button', {
          type: 'button', role: 'switch', 'aria-checked': floatOn, 'aria-label': t('ucFloatLabel'), title: t('ucFloatLabel'),
          onClick: function () { setFloatOn(!floatOn); },
          style: {
            flex: 'none', position: 'relative', width: '36px', height: '20px', padding: 0,
            borderRadius: '999px', border: '1px solid var(--dsw-alias-border-l2)',
            background: floatOn ? 'var(--dsw-alias-button-primary-fill, #4f6ef7)' : 'var(--dsw-alias-bg-layer-3)',
            cursor: 'pointer', transition: 'background .16s'
          }
        },
          react.createElement('span', {
            style: {
              position: 'absolute', top: '2px', left: floatOn ? '18px' : '2px', width: '14px', height: '14px',
              borderRadius: '999px', background: '#fff', boxShadow: '0 1px 2px rgba(0,0,0,0.3)', transition: 'left .16s'
            }
          })
        )
      );
    ...
    /** ── 侧边栏设置区域右侧:重启按钮(仅展开时 + 管理员)────────────────────────── */
    function RestartButton(props) {
      var t = props.t;
      var wide = props.wide;
      // 折叠态不渲染(设置区域宽度仅够放设置触发图标)
      if (!wide) return null;

      var adminState = react.useState(null); // null | true | false
      var isAdmin = adminState[0]; var setIsAdmin = adminState[1];
      var stState = react.useState({ busy: false, message: '', kind: '' });
      var st = stState[0]; var setState = stState[1];

      react.useEffect(function () {
        var alive = true;
        whoami().then(function (m) { if (alive) setIsAdmin(Boolean(m && m.admin)); });
        return function () { alive = false; };
      }, []);

      // 非管理员不显示(后端 /tenancy/restart 有二次鉴权)
      ...
      function doRestart() {
        ...
        setState({ busy: true, message: '', kind: '' });
        fetch('/tenancy/restart', { method: 'POST', credentials: 'same-origin' })
          .then(function (r) { return r.json().catch(function () { return {}; }); })
          .then(function (data) {
            if (data && data.ok) {
              setState({ busy: false, message: t('restartDone'), kind: 'ok' });
            } else {
              setState({ busy: false, message: t('restartFailed'), kind: 'err' });
            }
          })
          .catch(function () { setState({ busy: false, message: t('restartFailed'), kind: 'err' }); });
      }
      ...
      var restartIcon = react.createElement('svg', {
    ...
    function apply(ctx) {
      ...
      // 常驻用户中心入口:挂 root 级常驻槽位(sidebar.footer.action 始终挂载),
      // 组件以 position:fixed 渲染在 body 顶部居中,与挂载点无关;默认不可拖动,
      // 设置卡片打开「浮动入口」后才可拖拽。含工作区管理 / 会话管理 /
      // 修改密码 / 邀请码管理(管理员) / 登出
      ...
      // 侧边栏设置区域右侧:重启按钮(管理员可见;后端 /tenancy/restart 二次鉴权)
      ctx.slots.inject('sidebar.settings.action', function () {
        return ctx.slots.register({
