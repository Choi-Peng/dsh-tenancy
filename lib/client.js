...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function WorkspaceList(props) {
      ...
      react.useEffect(function () {
        var alive = true;
        // /tenancy/workspaces 提供旁车 owner 记录;connection.api.workspaces.list
        // 提供标题与会话数(服务端已按可见性过滤,与 owner 一览对齐)
        var wsPromise = api('GET', '/tenancy/workspaces');
        var metaPromise = Promise.resolve()
          .then(function () { return connection.api.workspaces.list({}); })
    ...
    function UserCenter(props) {
      ...
      return react.createElement('div', { 'data-uc': 'true' },
        react.createElement('button', {
          type: 'button', 'data-uc': 'true', title: t('ucOpen'),
        ...
        open ? react.createElement('div', {
          ...
          style: {
            position: 'fixed', top: '54px', right: '14px', zIndex: 9600,
